...
contains
  ...
  subroutine get_potential_field_potential_sphere(ixImin1,ixImin2,ixImin3,&
     ixImax1,ixImax2,ixImax3,x,potential,nth,nph,magnetogram,theta,phi,&
     r_sphere)
  ! PURPOSE: 
  ! Calculation scalar potential of potential field given
  ! Bz at photosphere (Schmidt 1964 NASSP). 
  ...
  ! OUTPUT: potential
    use mod_global_parameters
    integer, intent(in) :: ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,nth,&
       nph
    real*8, intent(in) :: x(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3,&
       1:ndim)
    ...
    real*8, intent(out) :: potential(ixImin1:ixImax1,ixImin2:ixImax2,&
       ixImin3:ixImax3)

    real*8 :: area(nth),distance(ixImin1:ixImax1,ixImin2:ixImax2,&
       ixImin3:ixImax3),dtheta_half,dphi,inv2pi
    integer :: ix1,ix2
    ...
    do ix2=1,nph,2
      do ix1=1,nth,2
        distance(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3)=sqrt(x(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3,1)**2+r_sphere**2-2.d0*x(ixImin1:ixImax1,&
           ixImin2:ixImax2,ixImin3:ixImax3,1)*r_sphere*(sin(x(ixImin1:ixImax1,&
           ixImin2:ixImax2,ixImin3:ixImax3,&
           2))*sin(theta(ix1))*cos(phi(ix2)-x(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3,3))+cos(x(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3,2))*cos(theta(ix1))))
        where(distance(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3)/=0.d0)
          distance(ixImin1:ixImax1,ixImin2:ixImax2,&
             ixImin3:ixImax3)=1.d0/distance(ixImin1:ixImax1,ixImin2:ixImax2,&
             ixImin3:ixImax3)
        end where
        potential(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3)=potential(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3)+inv2pi*magnetogram(ix1,&
           ix2)*distance(ixImin1:ixImax1,ixImin2:ixImax2,&
           ixImin3:ixImax3)*area(ix1)
      end do
    end do
    !$OMP END PARALLEL DO
  ...
  ! Only works for 3D Cartesian coordinates 
    ...
    real*8  :: block_Benergy(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ixMlo3:ixMhi3),&
        mype_Benergy
    real*8, allocatable :: tmp(:,:,:)
    integer :: iigrid, igrid, idir
    integer :: id,nc, lvl, ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3

    call get_potential_field_potential_mg()
    ixImin1=ixMlo1-1;ixImin2=ixMlo2-1;ixImin3=ixMlo3-1;
    ixImax1=ixMhi1+1;ixImax2=ixMhi2+1;ixImax3=ixMhi3+1;
    allocate(tmp(ixImin1:ixImax1,ixImin2:ixImax2,ixImin3:ixImax3))
    mype_Benergy=0.d0
    do iigrid=1,igridstail; igrid=igrids(iigrid);
      ...
      block_Benergy=0.d0
      do idir=1,ndim
        call gradient(mg%boxes(id)%cc(0:nc+1,0:nc+1,0:nc+1,mg_iphi),ixImin1,&
           ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,ixMlo1,ixMlo2,ixMlo3,ixMhi1,&
           ixMhi2,ixMhi3,idir,tmp)
        block_Benergy(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
           ixMlo3:ixMhi3)=block_Benergy(ixMlo1:ixMhi1,ixMlo2:ixMhi2,&
           ixMlo3:ixMhi3)+tmp(ixMlo1:ixMhi1,ixMlo2:ixMhi2,ixMlo3:ixMhi3)**2
      end do
      mype_Benergy=mype_Benergy+sum(0.5d0*block_Benergy(ixMlo1:ixMhi1,&
         ixMlo2:ixMhi2,ixMlo3:ixMhi3)*block%dvolume(ixMlo1:ixMhi1,&
         ixMlo2:ixMhi2,ixMlo3:ixMhi3))
    end do

    call MPI_ALLREDUCE(mype_Benergy,benergy,1,MPI_DOUBLE_PRECISION,MPI_SUM,&
       icomm,ierrmpi)
  ...
  subroutine get_potential_field_potential_mg()
    ...
    if(res > max_residual) call mpistop&
  ...
  subroutine multigrid_bc(box, nc, iv, nb, bc_type, bc)
    ...
    double precision :: wbn(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixGlo3:ixGhi3)
    double precision, allocatable :: xcoarse(:,:,:)
    integer :: iigrid,igrid,ix1,ix2,ix3,idir,ixbca,ixbcb,ixbcn,dlvl,wnc
    ...
    case(1)
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
        ...
        if(stagger_grid) then
          !wbn(ixbcn
          wbn(ixbcn,ixGlo2:ixGhi2,ixGlo3:ixGhi3)=block%ws(ixbcn,ixGlo2:ixGhi2,&
             ixGlo3:ixGhi3,idir)
        else
          wbn(ixbcn,ixGlo2:ixGhi2,ixGlo3:ixGhi3)=half*(block%w(ixbcn,&
             ixGlo2:ixGhi2,ixGlo3:ixGhi3,iw_mag(idir))+block%w(ixbcn+1,&
             ixGlo2:ixGhi2,ixGlo3:ixGhi3,iw_mag(idir)))
        end if
        ...
        if(xmina<rr(1,1,2) .and. xmaxa>rr(nc,1,2) .and. xminb<rr(1,1,&
           3) .and. xmaxb>rr(1,nc,3)) then
          do ix2=1,nc
            do ix1=1,nc
        ...
        else if(block%x(1,ixMlo2,1,2)>rmina .and. block%x(1,ixMhi2,1,&
           2)<rmaxa .and. block%x(1,1,ixMlo3,3)>rminb .and. block%x(1,1,ixMhi3,&
           3)<rmaxb) then
          ...
          do ix2=1,wnc
            do ix1=1,wnc
              xcoarse(ix1,ix2,1)=sum(block%x(1,&
                 (ix1-1)*2**dlvl+1+nghostcells:ix1*2**dlvl+nghostcells,1,&
                 2))/dble(2**dlvl)
              ...
              bc(ixbca,ixbcb)=sum(wbn(ixbcn,&
                 ...
                 (ix2-1)*2**dlvl+1+nghostcells:ix2*2**dlvl+&
                 nghostcells))/dble(2**(2*dlvl))
            end do
          end do
          deallocate(xcoarse)
        end if
    ...
    case(2)
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
        ...
        if(.not.block%is_physical_boundary(nb)) cycle
        if(stagger_grid) then
          wbn(ixGlo1:ixGhi1,ixbcn,ixGlo3:ixGhi3)=block%ws(ixGlo1:ixGhi1,ixbcn,&
             ixGlo3:ixGhi3,idir)
        else
          wbn(ixGlo1:ixGhi1,ixbcn,ixGlo3:ixGhi3)=half*(block%w(ixGlo1:ixGhi1,&
             ixbcn,ixGlo3:ixGhi3,iw_mag(idir))+block%w(ixGlo1:ixGhi1,ixbcn+1,&
             ixGlo3:ixGhi3,iw_mag(idir)))
        end if
        xmina=block%x(1,1,1,1)-0.5d0*rnode(rpdx1_,igrid)
        xmaxa=block%x(ixGhi1,1,1,1)+0.5d0*rnode(rpdx1_,igrid)
        xminb=block%x(1,1,1,3)-0.5d0*rnode(rpdx3_,igrid)
        xmaxb=block%x(1,1,ixGhi3,3)+0.5d0*rnode(rpdx3_,igrid)
        if(xmina<rr(1,1,1) .and. xmaxa>rr(nc,1,1) .and. xminb<rr(1,1,&
           3) .and. xmaxb>rr(1,nc,3)) then
          do ix2=1,nc
            do ix1=1,nc
        ...
        else if(block%x(ixMlo1,1,1,1)>rmina .and. block%x(ixMhi1,1,1,&
           1)<rmaxa .and. block%x(1,1,ixMlo3,3)>rminb .and. block%x(1,1,ixMhi3,&
           3)<rmaxb) then
          dlvl=node(plevel_,igrid)-box%lvl
          wnc=nc/2**dlvl
          allocate(xcoarse(wnc,wnc,2))
          do ix2=1,wnc
            do ix1=1,wnc
              xcoarse(ix1,ix2,1)=sum(block%x((ix1-1)*2**dlvl+1+&
                 nghostcells:ix1*2**dlvl+nghostcells,1,1,1))/dble(2**dlvl)
              xcoarse(ix1,ix2,2)=sum(block%x(1,1,&
                 (ix2-1)*2**dlvl+1+nghostcells:ix2*2**dlvl+nghostcells,&
                 3))/dble(2**dlvl)
              ixbca=ceiling((xcoarse(ix1,ix2,1)-rmina)/box%dr(1))
              ixbcb=ceiling((xcoarse(ix1,ix2,2)-rminb)/box%dr(3))
              bc(ixbca,ixbcb)=sum(wbn((ix1-1)*2**dlvl+1+&
                 nghostcells:ix1*2**dlvl+nghostcells,ixbcn,&
                 (ix2-1)*2**dlvl+1+nghostcells:ix2*2**dlvl+&
                 nghostcells))/dble(2**(2*dlvl))
            end do
          end do
    ...
    case(3)
      ...
      do iigrid=1,igridstail; igrid=igrids(iigrid);
        ...
        if(.not.block%is_physical_boundary(nb)) cycle
        if(stagger_grid) then
          wbn(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixbcn)=block%ws(ixGlo1:ixGhi1,&
             ixGlo2:ixGhi2,ixbcn,idir)
        else
          wbn(ixGlo1:ixGhi1,ixGlo2:ixGhi2,ixbcn)=half*(block%w(ixGlo1:ixGhi1,&
             ixGlo2:ixGhi2,ixbcn,iw_mag(idir))+block%w(ixGlo1:ixGhi1,&
             ixGlo2:ixGhi2,ixbcn+1,iw_mag(idir)))
        end if
        xmina=block%x(1,1,1,1)-0.5d0*rnode(rpdx1_,igrid)
        xmaxa=block%x(ixGhi1,1,1,1)+0.5d0*rnode(rpdx1_,igrid)
        ...
        if(xmina<rr(1,1,1) .and. xmaxa>rr(nc,1,1) .and. xminb<rr(1,1,&
          ...
          do ix2=1,nc
           ...
           1)<rmaxa .and. block%x(1,ixMlo2,1,2)>rminb .and. block%x(1,ixMhi2,1,&
           2)<rmaxb) then
          dlvl=node(plevel_,igrid)-box%lvl
          wnc=nc/2**dlvl
          allocate(xcoarse(wnc,wnc,2))
          do ix2=1,wnc
            do ix1=1,wnc
              ...
              xcoarse(ix1,ix2,2)=sum(block%x(1,&
                 (ix2-1)*2**dlvl+1+nghostcells:ix2*2**dlvl+nghostcells,1,&
                 2))/dble(2**dlvl)
              ixbca=ceiling((xcoarse(ix1,ix2,1)-rmina)/box%dr(1))
              ixbcb=ceiling((xcoarse(ix1,ix2,2)-rminb)/box%dr(2))
              bc(ixbca,ixbcb)=sum(wbn((ix1-1)*2**dlvl+1+&
                 nghostcells:ix1*2**dlvl+nghostcells,&
                 (ix2-1)*2**dlvl+1+nghostcells:ix2*2**dlvl+nghostcells,&
                 ixbcn))/dble(2**(2*dlvl))
            end do
          end do
