...
contains
  ...
  subroutine init_opal_table(tablename)
    ...
    path_table_dir = trim(AMRVAC_DIR)//"/src/rhd/OPAL_tables/"//trim(&
       tablename)
  ...
  subroutine set_opal_opacity(rho, temp, kappa)
    ...
    call get_kappa(Kappa_vals, logR_list, logT_list, logR_in, logT_in,&
        logKappa_out)
  ...
  subroutine read_table(R, T, K, filename)
    ...
    double precision, intent(out) :: K(iTmin:iTmax,iRmin:iRmax),&
        R(iRmin:iRmax), T(iTmin:iTmax)
  ...
  subroutine get_kappa(Kappa_vals, logR_list, logT_list, R, T, K)
    ...
    double precision, intent(in)  :: logR_list(iRmin:iRmax),&
    ...
    elseif (R .lt. minval(logR_list)) then
        ...
        call get_low_up_index(R, logR_list, iRmin, iRmax, low_R_index,&
            up_R_index)
    endif
    ...
    call interpolate_KRT(low_R_index, up_R_index, low_T_index, up_T_index,&
        logR_list, logT_list, Kappa_vals, R, T, K)
  ...
  subroutine interpolate_KRT(low_r, up_r, low_t, up_t, logR_list, logT_list,&
      Kappa_vals, R, T, kappa_interp)
