...
contains
  ...
  subroutine getlQgrid(lQgrid,ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,ixImax3,&
     ixOmin1,ixOmin2,ixOmin3,ixOmax1,ixOmax2,ixOmax3,qt,trelax,x,mode,vtim,&
     vlim)
    use mod_global_parameters
    integer, intent(in)             :: ixImin1,ixImin2,ixImin3,ixImax1,ixImax2,&
       ixImax3, ixOmin1,ixOmin2,ixOmin3,ixOmax1,ixOmax2,ixOmax3, mode, vtim,&
        vlim
    ...
    integer                         :: Bp,Bt1,Bt2,ix1,ix2,ix3,i
    ...
    case(1)
      if(qt .lt. trelax) then
        lQgrid(ixOmin1:ixOmax1,ixOmin2:ixOmax2,ixOmin3:ixOmax3)=0.d0
      else
        do i = 4, vtim
          if(qt .lt. trelax + tb1(i)) then
            ...
            do ix1 = ixOmin1,ixOmax1
              Bp = floor((x(ix1,ix2,ix3,1) - xprobmin1) / stt) + 1
              if (Bp .lt. 1)  Bp=1
              if(x(ix1,ix2,ix3,1) .lt. 0.d0) then
                lQgrid(ix1,ix2,ix3) = tl1 * (va1(Bp + &
                   (i-4)*vlim)*(one-va0)+va0) + tl2 * (va1(Bp + &
                   (i-3)*vlim)*(one-va0)+va0) + tl3 * (va1(Bp + &
        ...
        do i = 4, vtim
          if(qt .lt. trelax + tb2(i)) then
            ...
            do ix1 = ixOmin1,ixOmax1
              Bp = floor((x(ix1,ix2,ix3,1) - xprobmin1) / stt) + 1   
              if (Bp .lt. 1)  Bp=1
              if(x(ix1,ix2,ix3,1) .ge. 0.d0) then
                lQgrid(ix1,ix2,ix3) = tr1 * (va2(Bp + &
                   (i-4)*vlim)*(one-va0)+va0) + tr2 * (va2(Bp + &
                   (i-3)*vlim)*(one-va0)+va0) + tr3 * (va2(Bp + &
    ...
    case(2)
      ...
      do ix1 = ixOmin1,ixOmax1
          Bp = floor((x(ix1,ix2,ix3,1) - xprobmin1) / stt) + 1
          if(x(ix1,ix2,ix3,1) .lt. 0.d0) then
            lQgrid(ix1,ix2,ix3) = tr1 * (va1(Bp + Bt1*vlim)*(one-va0)+va0) + &
               tr3 * (va1(Bp+(Bt1+1)*vlim)*(one-va0)+va0)
          else
            lQgrid(ix1,ix2,ix3) = tr2 * (va2(Bp + Bt2*vlim)*(one-va0)+va0) + &
               tr4 * (va2(Bp+(Bt2+1)*vlim)*(one-va0)+va0)
          endif
      enddo
      enddo
      enddo
