...
contains
  ...
  subroutine init_cak_table(tabledir)
    ...
    call read_table(logD_list, logT_list, alpha_vals,&
        path_table_dir//"/al_TD")
    ...
    call read_table(logD_list, logT_list, kappae_vals,&
        path_table_dir//"/Ke_TD")
  ...
  subroutine set_cak_opacity(rho, temp, alpha_output, Qbar_output, Q0_output,&
      kappae_output)
    ...
    call get_val_comb(alpha_vals,Qbar_vals,Q0_vals,kappae_vals, logD_list,&
        logT_list, D_input, T_input, alpha_output, Qbar_output, Q0_output,&
        kappae_output)
  ...
  subroutine read_table(D, T, K, filename)
    ...
    double precision, intent(out) :: K(iDmin:iDmax,iTmin:iTmax),&
  ...
  subroutine get_val_comb(K1_vals,K2_vals,K3_vals,K4_vals, logD_list,&
      logT_list, D, T, K1, K2, K3, K4)
    ...
    double precision, intent(in)  :: logD_list(iDmin:iDmax),&
        logT_list(iTmin:iTmax)
    ...
    elseif (D .lt. minval(logD_list)) then
        ...
        call get_low_up_index(D, logD_list, iDmin, iDmax, low_D_index,&
            up_D_index)
    endif
    ...
    elseif (T .lt. minval(logT_list)) then
        ...
        call get_low_up_index(T, logT_list, iTmin, iTmax, low_T_index,&
            up_T_index)
    endif
  ...
  subroutine interpolate_KRT(low_r, up_r, low_t, up_t, logD_list, logT_list,&
      Kappa_vals, D, T, kappa_interp)
